...
contract TstPreICO {
     ...
     * External contracts
     */
    TstToken public tstToken = TstToken(0x0);
        ...
        returns (uint)
    {
        ...
        if (!tstToken.issueTokens(msg.sender, tokenCount)) {
        ...
        returns (uint)
    {
        ...
        if (!tstToken.issueTokens(beneficiary, _tokenCount)) {
            // Tokens could not be issued.
            throw;
        }
        return _tokenCount;
        ...
        returns (bool)
    {
        if (isICOActive == true) {
            ...
            if (!tstToken.issueTokens(multisig, founderBonus)) {
                // Tokens could not be issued.
                throw;
            }
        }
    ...
    function TstICO(address _multisig) {
        // Set founder address
        founder = msg.sender;
